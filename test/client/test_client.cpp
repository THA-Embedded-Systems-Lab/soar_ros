...
int main(int argc, char *argv[])
{
  ...
  std::shared_ptr<soar_ros::Client<example_interfaces::srv::AddTwoInts>> client =
      std::make_shared<TestClient>(agent->getSmlAgent(), node, "AddTwoIntsClient");
  agent->addClient(client, "AddTwoIntsClient");

  std::shared_ptr<soar_ros::Subscriber<std_msgs::msg::String>> trigger =
      std::make_shared<Trigger>(agent->getSmlAgent(), node, "Trigger");
  agent->addSubscriber(trigger);
