...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate MIMO sender/receiver log files."
    )
    parser.add_argument(
        "-d",
        "--run-dir",
    ...
    for log_file in log_files:
        ...
        sender_msgs, receiver_msgs, soar_decision_df = parse_log_file(
    ...
    if message_results:
        ...
        message_results.to_csv(
            os.path.join(data_dir, f"combined_message_results.csv"), index=False
        )

    if soar_decision_dfs:
        soar_decision_dfs = pd.concat(soar_decision_dfs)
        soar_decision_dfs.to_csv(
            os.path.join(data_dir, f"combined_soar_decision.csv"), index=False
        )
