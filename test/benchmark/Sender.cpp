...
private:
  ...
  void publish_header()
  {
    if (frame_counter_ < messages_to_send_)
    {
      ...
      for (int i = 0; i < num_inputs_; i++)
      {
        ...
        RCLCPP_INFO(this->get_logger(), "Published header to input%d: frame_id=%s, sec=%d, nanosec=%u", i,
    ...
    if (frame_counter_ == messages_to_send_)
    {
      ...
      shutdown_timer = this->create_wall_timer(std::chrono::seconds(2), [this]() {
