...
for F in "${FREQS[@]}"; do
  ...
  ros2 launch soar_ros benchmark.launch.py f:=${F} num_inputs:=${NUM_CHANNELS} num_outputs:=${NUM_CHANNELS} messages_to_send:=2000 auto_delete_soar_io_on_complete:=True
